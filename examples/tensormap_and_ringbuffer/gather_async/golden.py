"""
Golden reference for gather_async: multi-card TGET_ASYNC via SDMA, no computation.

Each rank has local src data (GATHER_COUNT elements). Root gathers
GATHER_COUNT from each rank into out: [rank0_data, rank1_data, ...].
Uses asynchronous SDMA DMA engine (GM-to-GM direct transfer).
"""
...
def generate_inputs(params: dict) -> list:
    """Return flat argument list.

    For requires_comm, params includes device_ctx_ptr, win_in_base,
    win_out_base, n_ranks, root, rank_id.
    For requires_sdma, params also includes sdma_workspace_ptr.
    """
