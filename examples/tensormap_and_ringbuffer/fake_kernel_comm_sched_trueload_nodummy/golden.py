"""
Golden reference for fake_kernel_comm_sched_trueload_nodummy.

This keeps the same input interface as fake_kernel_comm_sched_trueload so the
two cases can be benchmarked side-by-side with identical host-side arguments.
The orchestration ignores the dummy-task-related args.
"""
    ...
    if "device_ctx_ptr" in params and "win_in_base" in params and "win_out_base" in params:
        result.extend(
            [
                ...
                ("dummy_comm_bytes", ctypes.c_int64(_DUMMY_COMM_BYTES)),
            ]
        )
...
def post_run_collect(outputs: dict, params: dict) -> None:
    ...
    fname = _OUTPUTS_DIR / f"poll_counts_trueload_nodummy_{strategy_str}_gc{GATHER_COUNT}_r{n_ranks}.json"
    with open(fname, "w", encoding="utf-8") as f:
        json.dump(
            {
                "strategy": strategy_str,
                "case": "trueload_nodummy",
                "gather_count": GATHER_COUNT,
                "n_ranks": n_ranks,
                "n_iter": _N_ITER,
                "serialize_dummy_arg": _SERIALIZE_DUMMY,
                "dummy_comm_bytes_arg": _DUMMY_COMM_BYTES,
                "dummy_pingpong_bytes_arg": _DUMMY_PINGPONG_BYTES,
                "dummy_float_bytes": _FLOAT_BYTES,
