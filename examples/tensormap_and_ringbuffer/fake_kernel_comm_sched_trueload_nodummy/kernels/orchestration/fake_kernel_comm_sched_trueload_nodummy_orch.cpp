/**
 * trueload_nodummy orchestration:
 * - same single-AICore-domain setup as fake_kernel_comm_sched_trueload
...
void aicpu_orchestration_entry(PTO2Runtime* rt, uint64_t* args, int arg_count) {
    ...
    uint64_t dummy_comm_bytes = (arg_count > 15) ? args[15] : 0;
    if (n_iter <= 0) n_iter = DEFAULT_N_ITER;
    (void)serialize_dummy;
    (void)dummy_comm_bytes;

    int gather_count = static_cast<int>(size_src / static_cast<int64_t>(sizeof(float)));

    LOG_INFO(rt, "trueload_nodummy: strategy=%d gather_count=%d n_ranks=%d rank=%d n_iter=%d",
    ...
    LOG_INFO(rt, "trueload_nodummy tasks submitted");
}
