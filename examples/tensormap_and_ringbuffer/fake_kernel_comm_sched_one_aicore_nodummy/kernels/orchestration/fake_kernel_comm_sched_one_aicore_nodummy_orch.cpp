/**
 * one_aicore_nodummy orchestration:
 * - same single-AICore-domain setup as fake_kernel_comm_sched_one_aicore
 * - same argument interface for A/B comparison
 * - only difference: no extra dummy communication task is submitted
 */
...
void aicpu_orchestration_entry(PTO2Runtime* rt, uint64_t* args, int arg_count) {
    ...
    (void)dummy_comm_scale;

    int gather_count = static_cast<int>(size_src / static_cast<int64_t>(sizeof(float)));

    LOG_INFO(rt, "one_aicore_nodummy: strategy=%d gather_count=%d n_ranks=%d rank=%d n_iter=%d",
    ...
    PTO2_SCOPE(rt) {
        ...
        for (int iter = 0; iter < n_iter; ++iter) {
            ...
            // Keep iterations serialized behind the main gather only.
            prev_iter_sync = gather_out;
        }
    ...
    LOG_INFO(rt, "one_aicore_nodummy tasks submitted");
}
