"""
Golden reference for fake_kernel_comm_sched_trueload.

This case keeps the one-AICore dual-AIV scheduling setup, but the main
communication flow uses explicit load/store kernels instead of gather kernels.
"""
...
def post_run_collect(outputs: dict, params: dict) -> None:
    ...
    fname = _OUTPUTS_DIR / f"poll_counts_trueload_{strategy_str}_gc{GATHER_COUNT}_r{n_ranks}.json"
        ...
        json.dump(
            {
                ...
                "dummy_pingpong_bytes": _DUMMY_PINGPONG_BYTES,
