 ...
 * Phase 2: WindowMemCopyIn → CommBarrier → TGATHER → WindowMemCopyOut (root only)
 *
 * Args layout: same as paged_attention but host_out is attn_out, plus:
 *   host_gather_out (root only), device_ctx_ptr, win_in_base, win_out_base,
 *   n_ranks, root, rank_id. Expect arg_count >= 16 for comm args.
 */
...
int build_paged_attention_gather_graph(Runtime* runtime, uint64_t* args, int arg_count) {
    if (arg_count < 16) {
        std::cerr << "build_paged_attention_gather_graph: Expected at least 16 args, got " << arg_count << '\n';
        return -1;
    }
    ...
    void* host_gather_out = reinterpret_cast<void*>(args[6]);
    ...
    size_t gather_out_size = static_cast<size_t>(args[14]);
    ...
    if (!dev_query || !dev_key_cache || !dev_value_cache || !dev_attn_out) {
    ...
    void* dev_gather_out = nullptr;
    if (rank_id == root) {
        dev_gather_out = runtime->host_api.device_malloc(gather_out_size);
        if (!dev_gather_out) {
            runtime->host_api.device_free(dev_attn_out);
            return -1;
        }
        runtime->record_tensor_pair(host_gather_out, dev_gather_out, gather_out_size);
    }
    ...
    runtime->host_api.copy_to_device(reinterpret_cast<void*>(barrier_base), zeros, barrier_size);
    ...
    runtime->add_successor(t_wmin, t_barrier);
    ...
    if (dev_gather_out != nullptr) {
        uint64_t args_wmout[3] = {
            reinterpret_cast<uint64_t>(dev_gather_out),
            win_dst,
