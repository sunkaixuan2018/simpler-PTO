...
Each rank computes paged attention on its own Q/K/V, then root gathers
first GATHER_COUNT elements from each rank's attn_out into gather_out.
"""
...
__outputs__ = ["attn_out", "gather_out"]
RTOL = 1e-2
ATOL = 1e-2
ALL_CASES = {"Default": {}}
...
def generate_inputs(params: dict) -> list:
    ...
    gather_out = torch.zeros(n_ranks * GATHER_COUNT, dtype=torch.float32)  # root only, but all ranks need buffer
    result = [
        ("query", query), ("key_cache", key_cache), ("value_cache", value_cache),
        ("block_table", block_table_flat), ("context_lens", context_lens),
        ("attn_out", attn_out), ("gather_out", gather_out), ("config", config),
        ...
        ("size_gather_out", ctypes.c_int64(gather_out.nbytes)), ("size_config", ctypes.c_int64(config.nbytes)),
    ]
...
def compute_golden(tensors: dict, params: dict) -> None:
    ...
    if rank_id == root:
        gather_np = tensors["gather_out"].cpu().numpy() if hasattr(tensors["gather_out"], 'cpu') else np.asarray(tensors["gather_out"])
        for r in range(n_ranks):
            attn_r = _compute_rank_attn(r, block_table, context_lens_t, total_blocks)
            flat_r = attn_r.flatten().numpy()
            gather_np[r * GATHER_COUNT : (r + 1) * GATHER_COUNT] = flat_r[:GATHER_COUNT]
