"""
CodeRunner - Simplified test framework for PTO runtime tests (multi-card version).
...
Usage:
    ...
    # In Python
    from multi_card_code_runner import CodeRunner
...
class CodeRunner:
    ...
    def __init__(
        ...
        # Setup logging if not already configured (e.g., when used directly, not via multi_card_run_example.py)
        ...
        self.rank_id = rank_id if rank_id is not None else (device_id if device_id is not None else 0)
    ...
    def run(self, comm_context: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        if self._skip_build:
            ...
            if pto_isa_root is None:
                raise EnvironmentError(
                    ...
                    "Please set PTO_ISA_ROOT or PTO_COMM_ISA_ROOT to the pto-isa/pto-comm-isa root."
                )
            ...
            if self.requires_comm:
                comm_include = self.project_root / "examples" / "scripts" / "comm_include"
                if comm_include.exists():
                    runtime_include_dirs.append(str(comm_include))
...
def create_code_runner(kernels_dir, golden_path, device_id=None, platform="a2a3",
                       ...
                       compiled_artifacts=None, prebuilt_dir=None, rank_id=None):
...
def run_on_device_comm(
    ...
    from bindings import bind_host_binary, set_device, get_aicpu_stream

    # Ensure HCCL comm resources are created on the same AICPU stream
    # used by runtime/device side scheduler in this process.
    bind_host_binary(artifacts["host_binary"])
    set_device(device_id)
    runtime_stream = get_aicpu_stream()
