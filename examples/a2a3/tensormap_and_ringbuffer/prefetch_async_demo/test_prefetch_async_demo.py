...
Exercises the runtime-injected SDMA workspace: the host provisions the PTO-ISA
async-SDMA workspace on the first run of a declaring callable, caches it for the
runner's device lifetime, and injects its address into every kernel's
GlobalContext, so the kernel obtains it via
``get_dma_workspace(args, DMA_WORKSPACE_SDMA)`` -- no workspace is threaded as a
user arg. The kernel prefetches ``in`` into L2, waits on the returned event, then
copies ``in`` to ``out``.
