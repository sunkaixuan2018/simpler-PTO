...
# -----------------------------------------------------------------------------------------------------------
# Start the single-host 16-device Fabric smoke through OpenMPI or MPICH mpirun.
set -euo pipefail
...
for key in MPI_IMPLEMENTATION MPI_RUN MPI_TIMEOUT_SECONDS RANKS_PER_HOST \
    MASTER_HOST MASTER_SCRIPT_DIR MASTER_ASCEND_HOME_PATH MASTER_LOG_DIR; do
    require_config "${key}"
done
...
for numeric_key in MPI_TIMEOUT_SECONDS RANKS_PER_HOST; do
...
if [ "${RANKS_PER_HOST}" -ne 16 ]; then
    echo "[a3-smoke] single-host smoke requires RANKS_PER_HOST=16" >&2
    exit 1
fi
...
if [ ! -f "${MASTER_SCRIPT_DIR}/run_local.sh" ]; then
    echo "[a3-smoke] run_local.sh not found under ${MASTER_SCRIPT_DIR}" >&2
    exit 1
fi
...
HOSTFILE="$(mktemp "${TMPDIR:-/tmp}/a3-smoke-1host-hostfile.XXXXXX")"
trap 'rm -f "${HOSTFILE}"' EXIT
if [ "${MPI_IMPLEMENTATION}" = "openmpi" ]; then
    printf '%s slots=%s\n' "${MASTER_HOST}" "${RANKS_PER_HOST}" > "${HOSTFILE}"
else
    printf '%s\n' "${MASTER_HOST}" > "${HOSTFILE}"
fi

echo "[a3-smoke] mpi=${MPI_IMPLEMENTATION} launcher=${MPI_RUN}"
echo "[a3-smoke] host=${MASTER_HOST} script=${MASTER_SCRIPT_DIR}"
echo "[a3-smoke] mode=fabric layout=single-host-16 ranks=${RANKS_PER_HOST} peers=0-8,...,7-15"
...
if [ "${MPI_IMPLEMENTATION}" = "openmpi" ]; then
    ...
    timeout "${MPI_TIMEOUT_SECONDS}" "${MPI_RUN}" \
        ...
        bash "${MASTER_SCRIPT_DIR}/run_local.sh" fabric "${MASTER_ASCEND_HOME_PATH}" \
        "${MASTER_LOG_DIR}" single-host-16
else
    timeout "${MPI_TIMEOUT_SECONDS}" "${MPI_RUN}" -f "${HOSTFILE}" \
        -ppn "${RANKS_PER_HOST}" -np "${RANKS_PER_HOST}" \
        bash "${MASTER_SCRIPT_DIR}/run_local.sh" fabric "${MASTER_ASCEND_HOME_PATH}" \
        "${MASTER_LOG_DIR}" single-host-16
fi
