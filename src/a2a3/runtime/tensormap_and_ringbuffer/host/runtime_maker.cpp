...
extern "C" int init_runtime_impl(Runtime *runtime, const ChipCallable *callable, const ChipStorageTaskArgs *orch_args) {
    ...
    // SDMA async context initialization (controlled by PTO2_ENABLE_SDMA env var)
#if PTO2_HAS_SDMA_WORKSPACE_MANAGER
    {
        const char* env_sdma = std::getenv("PTO2_ENABLE_SDMA");
        if (env_sdma && env_sdma[0] == '1' && env_sdma[1] == '\0') {
            LOG_INFO("SDMA async context init requested (PTO2_ENABLE_SDMA=1)");
            ...
            if (sdma_manager.Init()) {
                ...
                runtime->set_async_context_addr(PTO2_ASYNC_ENGINE_SDMA, ws_addr);
                LOG_INFO("SDMA async context initialized: addr=0x%lx", (unsigned long)ws_addr);
            } else {
                LOG_WARN("SDMA async context initialization failed, continuing without SDMA support");
                runtime->set_async_context_addr(PTO2_ASYNC_ENGINE_SDMA, 0);
            }
        }
    }
#else
